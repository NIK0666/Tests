...
void UArticyFlowPlayer::SetOwner(UArticyPlayerManager* InOwner)
{
	Owner = InOwner;
}

UArticyPlayerManager* UArticyFlowPlayer::GetOwner()
{
	return Owner;
}
...
void UArticyFlowPlayer::SetCursorTo(TScriptInterface<IArticyFlowObject> Node)
{
	if(!Node.GetObject())
	{
		UE_LOG(LogArticyRuntime, Warning, TEXT("Could not set cursor in flow player: invalid node"));
		return;
	}
...
bool UArticyFlowPlayer::Play(int BranchIndex)
{
...
{
	return UArticyDatabase::Get(Owner->GetWorldContext());
}
...
UArticyGlobalVariables* UArticyFlowPlayer::GetGVs() const
{
	...
	return UArticyGlobalVariables::GetDefault(Owner->GetWorldContext());
}
...
UObject* UArticyFlowPlayer::GetMethodsProvider() const
{
	...
	if(ensure(provider))
	{
		...
		if(!UserMethodsProvider || !ensure(UserMethodsProvider->GetClass()->ImplementsInterface(provider)))
		{
			...
			{
				UObject* manager = Owner;
				if(ensure(manager))
				{
					if(manager->GetClass()->ImplementsInterface(provider))
					{
						UserMethodsProvider = manager;
					}
				}
			}
		}
	...
	else
	{
		...
		if(!bSubmerged)
		{
			...
			else
			{
				...
				//NIKO Detect custom condition
				if (auto ConditionNode = Cast<UArticyCondition>(Node))
				{
					if (OnCustomConditionSelected.IsBound())
					{
						int SelectedPin = -1;
						OnCustomConditionSelected.ExecuteIfBound(ConditionNode, SelectedPin);
						ConditionNode->ExploreCustom(this, OutBranches, Depth + 1, SelectedPin);
					}
					else
					{
				...
				else
				{
					...
					if (const auto HubNode = Cast<UArticyHub>(Node))
					{
...
void UArticyFlowPlayer::UpdateAvailableBranchesInternal(bool Startup)
{
	...
	else
	{
		...
		//NIKO Sort branches
		TArray<FArticyBranch> SortedBranches = AvailableBranches;
		SortedBranches.Sort([](const FArticyBranch& ip1, const FArticyBranch& ip2) {

			const auto obj1 = ip1.GetTarget().GetObject();
			const auto obj2 = ip2.GetTarget().GetObject();

			IArticyObjectWithPosition* obj_pos1 = Cast<IArticyObjectWithPosition>(obj1);
			IArticyObjectWithPosition* obj_pos2 = Cast<IArticyObjectWithPosition>(obj2);
			...
			return  obj_pos1->GetPosition().Y < obj_pos2->GetPosition().Y;
		});

		//broadcast and return result
		if (AvailableBranches.IsEmpty())
		{
			OnBranchesDone.ExecuteIfBound();
		}
		else
		{
			if (Cast<UArticyNode>(Cursor.GetObjectRef())!= nullptr)
			{
				OnPlayerPaused.ExecuteIfBound(Cursor);
			}
			
			OnBranchesUpdated.ExecuteIfBound(SortedBranches);
		}
		CurrentHubNode = nullptr;
	}
}
...
void UArticyFlowPlayer::PlayBranch(const FArticyBranch& Branch)
{
	...
	for(auto node : Branch.Path)
	{
		...
		//NIKO Detect instruction
		if (UArticyInstruction* ArticyInstruction = Cast<UArticyInstruction>(node.GetObject()))
		{
			OnInstructionExecuted.ExecuteIfBound(ArticyInstruction);
		}

		//NIKO Detect condition
		if (UArticyCondition* ArticyCondition = Cast<UArticyCondition>(node.GetObject()))
		{
			int32 SelectedPin = -1;
			OnCustomConditionSelected.ExecuteIfBound(ArticyCondition, SelectedPin);
			OnConditionExecuted.ExecuteIfBound(ArticyCondition, SelectedPin == 0);
		}
	}
